...
- Audited payroll export packages for locked/released payroll runs, including configurable bank-transfer column templates, token-vault/KMS/bank-format verification posture, and configurable accounting-journal summaries with content hashes and redacted audit metadata.
...
HR admins can open `/hr/payroll-accounting` to map payroll export summaries to the company chart of accounts, then open `/hr/payroll-exports` after payroll lock/release to generate audited bank-transfer drafts and accounting-journal packages. Payment destinations are tracked with masked/hash data, bank drafts use the configured column order, and production readiness requires token-vault, KMS, verified customer bank format, account token, and amount columns.
