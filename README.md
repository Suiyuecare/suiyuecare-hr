...
- Payroll close tracks the active Taiwan labor/payroll rule version used by each draft. If rules are pending legal review, marked for recalculation after a company override, or contain non-official/invalid legal source URLs, payroll lock is blocked until HR recalculates with the reviewed active version and HTTPS official `.gov.tw` sources.
