...
- AI usage logging stores category, actor, referenced record IDs, and output/prompt hashes without raw sensitive prompts. In database mode, AI audit writes fail closed instead of falling back to demo audit state, so Copilot actions cannot appear successful without durable tenant audit evidence.
