...
- 用 `pnpm pilot:go-no-go` 做試用開跑前總檢查，必須同時通過 acceptance、Day 0、匯入預檢、邀請 readiness 與證據掃描；再用 `pnpm pilot:workflow-readiness` 分辨核心流程是 production-ready 還是只有 rehearsal。
