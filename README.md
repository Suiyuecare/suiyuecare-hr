...
- Pilot invitation readiness now embeds the core workflow Gate, so HR sees Day 0, Day 1, Day 3, Day 7, and Day 14 evidence gaps for clock-in, leave, manager approval, announcements, monthly close, payslip access, and sensitive-data guardrails before inviting real employees. Owner/HR can run the preflight access review directly from this page; it verifies payroll dashboard and payslip permission boundaries and writes hash-only evidence without reading salary, bank account, national ID, or health values.
