...
- 前端員工日常使用：手機優先首頁、今日班表與打卡狀態、上下班打卡、請假、加班、補打卡、公告簽收、表單送出、申請進度時間軸、薪資單自助查看、員工訓練與公司規章確認。
...
- 試用與上線工具：Supabase private schema 驗證、Vercel production env 草稿、20-50 人 pilot 匯入模板、匯入預檢、身份/SSO 匯入、邀請 readiness、每日狀態、證據掃描、go/no-go 開跑總檢查、兩週試用結案檢查。
...
- `HR_ONE_AUTH_LOGIN_URL`: HTTPS company SSO login URL shown on `/auth/required`; placeholders, demo/example hosts, and local URLs are blocked.
- For Supabase Auth as the pilot IdP, use issuer `https://<project-ref>.supabase.co/auth/v1`, JWKS `https://<project-ref>.supabase.co/auth/v1/.well-known/jwks.json`, audience `authenticated`, and configure `HR_ONE_AUTH_DEFAULT_TENANT` / `HR_ONE_AUTH_DEFAULT_COMPANY` for the single-company pilot tenant when tokens do not carry custom tenant/company claims.
...
To connect Vercel production to Supabase, run `pnpm vercel:create-production-env-draft` to create a gitignored `.env.vercel.production` draft with generated local secrets, then replace every `REPLACE_WITH_*` placeholder with real production values. At minimum this requires the server-side Supabase Postgres `DATABASE_URL` with `?schema=hr_one`, production OIDC issuer/login/JWKS details, and real backup/restore drill evidence.

Before the operator-managed values are available, `pnpm vercel:bootstrap-known-env -- --env-file=.env.vercel.production` can dry-run a safe subset of known Production variables and generated secrets. It skips `DATABASE_URL`, OIDC provider/issuer/login/JWKS placeholders, vault references, and restore-drill evidence, and prints only key names plus sensitivity type. Pass `--apply` only when you intentionally want those known values written to Vercel; a later production deployment is still blocked until the operator-managed values are configured.
