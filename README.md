...
Owners can open the Finance-style `/settings/file-storage` workspace to configure file storage posture, including object storage provider, bucket, base prefix, KMS reference, lifecycle policy reference, allowed MIME types, maximum size, malware scan requirement, signed URL TTL, retention days, and verification evidence. Secrets stay in the provider vault and are not stored in this app. Launch readiness requires non-demo storage, KMS, bucket lifecycle policy, malware scanning, and a verified smoke-test status. Database-mode storage reads, writes, and upload reservations fail closed instead of silently falling back to demo object storage.
