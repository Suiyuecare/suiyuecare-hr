...
async function main() {
  ...
  const employmentTerm = await prisma.employeeEmploymentTerm.create({
    data: {
      ...
      contractLifecycleSummaryHash: hash(`${employeeOne.id}:contract-lifecycle`),
      severancePensionBonusSummaryHash: hash(`${employeeOne.id}:severance-pension-bonus`),
      mealLodgingToolCostSummaryHash: hash(`${employeeOne.id}:meal-lodging-tool-cost`),
      safetyHealthSummaryHash: hash(`${employeeOne.id}:safety-health`),
      trainingSummaryHash: hash(`${employeeOne.id}:training`),
      disasterCompensationSicknessSummaryHash: hash(`${employeeOne.id}:disaster-compensation-sickness`),
      disciplineSummaryHash: hash(`${employeeOne.id}:discipline`),
      rewardDisciplineSummaryHash: hash(`${employeeOne.id}:reward-discipline`),
      rightsObligationsSummaryHash: hash(`${employeeOne.id}:rights-obligations`),
