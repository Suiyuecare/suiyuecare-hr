...
test("Owner 可以用中文權限中樞邀請帳號並綁定員工", async ({ page }) => {
  ...
  await expect(page.getByLabel("正式登入 Gate 摘要").getByText("租戶 API 邊界")).toBeVisible();
  await expect(page.locator("#tenant_api_boundary")).toContainText("API 租戶隔離與服務層邊界");
...
test("HR 人事主檔工作台提供中文 Finance 風格總覽且主管只看團隊", async ({ page }) => {
  ...
  await expect(page.getByLabel("職務架構閉環").getByRole("heading", { name: "職務架構閉環" })).toBeVisible();
  await expect(page.getByLabel("職務架構訊號板").getByText("員工對應")).toBeVisible();
  await expect(page.getByLabel("職務架構下一步").getByText("把員工對應到標準職務")).toBeVisible();
...
test("兩週試用核心流程可從 UI 完成", async ({ page }) => {
  ...
  await expect(customReportJobs.getByText("待第二人覆核", { exact: true })).toBeVisible();
  await expect(customReportJobs.getByText("下載前需要第二人覆核")).toBeVisible();
  await expect(customReportJobs.getByRole("button", { name: "等待第二人" })).toBeDisabled();
  ...
  await expect(ownerReportJobs.getByText("待第二人覆核", { exact: true })).toBeVisible();
  await ownerReportJobs.getByLabel("覆核備註代碼").fill("REV-E2E-REPORT");
  await ownerReportJobs.getByRole("button", { name: "核准報表覆核" }).click();
  await expect(page).toHaveURL(/\/hr\/reports\?success=report-review#report-jobs$/);
