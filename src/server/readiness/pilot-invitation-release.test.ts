...
describe("pilot invitation release", () => {
  it("releases invitations only when database, go/no-go, invite readiness, rollout kit, and privacy checks pass", () => {
    const report = buildPilotInvitationReleaseReport({
      ...
      rolloutKitReport: {
        path: "/tmp/hr-one-pilot-rollout-kit.md",
        content: rolloutKitMarkdown("ready", 8, 3, "PASS"),
      },
    });
    ...
    expect(report.evidenceHashes).toHaveLength(4);
  ...
  it("blocks missing or non-ready reports before employee invitations", () => {
    const report = buildPilotInvitationReleaseReport({
      ...
      rolloutKitReport: {
        path: "/tmp/hr-one-pilot-rollout-kit.md",
        content: rolloutKitMarkdown("blocked", 12, 4, "BLOCK"),
      },
    });

    expect(report.status).toBe("blocked");
    expect(report.blockers).toBe(4);
  ...
  it("accepts JSON reports from CLI --json output", () => {
    const report = buildPilotInvitationReleaseReport({
      ...
      rolloutKitReport: {
        path: "rollout-kit.json",
        content: JSON.stringify({
          status: "ready",
          estimatedTrainingMinutes: 8,
          maxEmployeeTaskSteps: 3,
          checks: [{ id: "training_time", status: "pass" }],
        }),
      },
    });
  ...
  it("blocks invitation release when rollout training is too long or tasks are too deep", () => {
    const report = buildPilotInvitationReleaseReport({
      productionDatabaseReport: {
        path: "/tmp/hr-one-production-database-gate.md",
        content: productionDatabaseMarkdown("ready", "ready", "ready"),
      ...
      rolloutKitReport: {
        path: "/tmp/hr-one-pilot-rollout-kit.md",
        content: rolloutKitMarkdown("ready", 11, 4, "PASS"),
      },
    });

    expect(report.status).toBe("blocked");
    expect(report.checks.find((check) => check.id === "rollout_kit_report")).toMatchObject({
      status: "block",
    });
    expect(formatPilotInvitationReleaseMarkdown(report)).toContain("Fix rollout kit blockers");
  });
});
...
function rolloutKitMarkdown(
  ...
  return [
    ...
    "",
    `Status: ${status}`,
    `Estimated employee training: ${estimatedTrainingMinutes} minute(s)`,
    `Max employee task steps: ${maxEmployeeTaskSteps}`,
    "",
    "## Checks",
    "",
    `- [${check}] Training time`,
