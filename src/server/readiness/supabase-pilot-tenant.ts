...
export function buildSupabasePilotTenantSeedPlan(
  ...
  const sqlSections = [
    ...
    upsertRows("CompanyFileStorageSetting", [
      ...
      "supabase://suiyuecare-hrone-documents/lifecycle/hr-documents-7y",
      true,
      10,
      25,
    ...
    insertRows("EmployeeLaborRosterProfile", [
      ...
      "wageInfoHash",
      "laborInsuranceEnrollmentDate",
      "rewardDisciplineSummaryHash",
      "injurySicknessSummaryHash",
      "otherNecessaryItemsHash",
      "rosterSourceRef",
      "requiredFieldsJson",
      "missingFieldsJson",
      "verificationStatus",
      ...
      "verified-summary-only",
      hash(`labor-roster:wage-info:${employee.employeeNo}`),
      referenceDate,
      hash(`labor-roster:reward-discipline:${employee.employeeNo}`),
      hash(`labor-roster:injury-sickness:${employee.employeeNo}`),
      hash(`labor-roster:other-necessary-items:${employee.employeeNo}`),
      ...
      '  "wageInfoHash" = EXCLUDED."wageInfoHash",',
      '  "laborInsuranceEnrollmentDate" = EXCLUDED."laborInsuranceEnrollmentDate",',
      '  "rewardDisciplineSummaryHash" = EXCLUDED."rewardDisciplineSummaryHash",',
      '  "injurySicknessSummaryHash" = EXCLUDED."injurySicknessSummaryHash",',
      '  "otherNecessaryItemsHash" = EXCLUDED."otherNecessaryItemsHash",',
...
export function buildSupabasePilotTenantVerificationSql(schemaName = "hr_one", tenantSlug = pilotTenantSlug): string {
  ...
  return [
    ...
    "  (SELECT count(*)::int FROM \"CompanyFileStorageSetting\" cfss JOIN target ON target.company_id = cfss.\"companyId\" WHERE cfss.provider <> 'demo_object_storage' AND nullif(btrim(cfss.\"bucketName\"), '') IS NOT NULL AND nullif(btrim(cfss.\"kmsKeyRef\"), '') IS NOT NULL AND nullif(btrim(cfss.\"lifecyclePolicyRef\"), '') IS NOT NULL AND lower(cfss.\"lifecyclePolicyRef\") LIKE '%' || lower(cfss.\"bucketName\") || '%' AND cfss.\"lifecyclePolicyRef\" ~* '(lifecycle|retention|archive|保留|封存|保存)' AND cfss.\"lifecyclePolicyRef\" !~* '(secret|token|password|credential|private[_-]?key|access[_-]?key|sk[_-])' AND (cfss.provider = 'custom' OR (cfss.provider = 's3' AND lower(cfss.\"lifecyclePolicyRef\") LIKE ANY (ARRAY['s3://%', 'aws://%'])) OR (cfss.provider = 'r2' AND lower(cfss.\"lifecyclePolicyRef\") LIKE ANY (ARRAY['r2://%', 'cloudflare-r2://%'])) OR (cfss.provider = 'gcs' AND lower(cfss.\"lifecyclePolicyRef\") LIKE ANY (ARRAY['gcs://%', 'gs://%'])) OR (cfss.provider = 'azure_blob' AND lower(cfss.\"lifecyclePolicyRef\") LIKE ANY (ARRAY['azure://%', 'azure-blob://%', 'azblob://%'])) OR (cfss.provider = 'supabase_storage' AND lower(cfss.\"lifecyclePolicyRef\") LIKE ANY (ARRAY['supabase://%', 'supabase-storage://%']))) AND cfss.\"retentionDays\" >= 1825 AND cfss.\"signedUrlTtlMinutes\" BETWEEN 1 AND 15 AND cfss.\"malwareScanningRequired\" = TRUE AND cfss.\"verificationStatus\" = 'verified' AND cfss.\"lastVerifiedAt\" IS NOT NULL) AS \"verifiedFileStorageCount\",",
