...
export function buildSupabasePilotTenantVerificationSql(schemaName = "hr_one", tenantSlug = pilotTenantSlug): string {
  ...
  return [
    ...
    "  (SELECT count(*)::int FROM \"CompanyFileStorageSetting\" cfss JOIN target ON target.company_id = cfss.\"companyId\" WHERE cfss.provider <> 'demo_object_storage' AND cfss.\"kmsKeyRef\" IS NOT NULL AND cfss.\"lifecyclePolicyRef\" IS NOT NULL AND cfss.\"verificationStatus\" = 'verified' AND cfss.\"lastVerifiedAt\" IS NOT NULL) AS \"verifiedFileStorageCount\",",
