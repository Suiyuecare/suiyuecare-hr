...
export function buildBetaPilotReadinessReport(input: BetaPilotReadinessInput): BetaPilotReadinessReport {
  ...
  const items: BetaPilotReadinessItem[] = [
    ...
    },
    {
      ...
      area: "Security",
      title: "試用租戶、登入與角色分流",
      status: combinedStatus(launchItems, ["database", "tenant_seed", "security", "sso_identities", "access_cutover"]),
      detail: launchDetail(launchItems, ["database", "tenant_seed", "security", "sso_identities", "access_cutover"]),
