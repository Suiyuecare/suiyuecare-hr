...
- `/settings/pilot-invite-readiness` shows the pre-invitation gate for login identity, role coverage, manager lines, 14-day schedules, leave balances, and self-only payslip visibility without exposing names, emails, salaries, bank accounts, SSO subjects, or private notes. It now also embeds the core workflow Gate for Day 0, Day 1, Day 3, Day 7, and Day 14 evidence gaps before HR sends real employee invitations, and lets Owner/HR run the preflight access review directly from the page with hash-only evidence.
