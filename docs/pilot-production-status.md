...
5. Run `pilot:invite-readiness` and `pilot:go-no-go` for the actual customer tenant before inviting staff; `pilot:go-no-go` now includes core workflow readiness, while standalone `pilot:workflow-readiness --require-production-evidence` should be used during Day 3, Day 7, and Day 14 evidence reviews.
